...
class WebformCompositeYukon extends WebformMultiple {
          ...
          foreach($value as $item) {
            ...
            foreach ($item as $key => $data) {
              ...
              if (!empty(self::$temp_items[$index]['_item_'][$key])) {
                switch ($data['#type']) {
                  case 'select':
                    if(array_key_exists(self::$temp_items[$index]['_item_'][$key], $data['#options'])){
                      ...
                      foreach($data['#options'] as $group => $options){
                        if(is_array($options)){
                          if(array_key_exists(self::$temp_items[$index]['_item_'][$key], $options)){
                             $element['records']['children-'.t(strval($index))]['child-'.t(strval($subIndex))] = [
                              '#type' => 'container',
                              '#markup' => '<p><strong>' . $data['#title'] . '</strong> ' . t(json_encode($data['#options'][$group][self::$temp_items[$index]['_item_'][$key]])) . '</p>',
                            ];
                          }
                        }
                      }
 ...
 \Drupal::logger("notice")->notice( json_encode($default_values));
